"""Local-only import, Linux rendering, atomic versions and traceable delivery."""
    ...
    try:
        with olefile.OleFileIO(BytesIO(data)) as ole:
            ...
            if not ole.exists("WordDocument"):
                raise FormatError("未发现 WordDocument 流；不能按 DOC 转换。")
            ...
            if len(header) < 12 or int.from_bytes(header[10:12], "little") & 0x8100:
                raise FormatError("DOC 加密或混淆保护，无法转换。")
...
class EditingSession:
    ...
    @classmethod
    def load(cls, name, data, *, renderer=render_document, converter=_convert):
        ...
        version = Version("original", stem + ("_工作副本.docx" if extension == ".doc" else ".docx"), baseline, None)
        return cls(name, data, {"original": version}, conversion=conversion, renders=renders, next_number=number, renderer=renderer)

    def model(self):
        v = self.versions[self.current]
        return build_model(v.data, v.id, {int(k): value for k, value in json.loads(v.roles_json).items()})
    ...
    def preview(self, version_id=None):
        ...
        if key not in self.renders:
            self.renders[key] = self.renderer(v.data, "docx")
    ...
    def execute(self, model, plan, *, environment=None):
        ...
        key = digest((plan.plan_hash + ENGINE_VERSION + encode(env)).encode())
        if key in self.jobs:
        ...
        old_key = digest((model.source_hash + encode(env)).encode())
        new_key = digest((digest(output) + encode(env)).encode())
        before = self.renders.get(old_key) or self.renderer(model.data, "docx")
        after = self.renders.get(new_key) or self.renderer(output, "docx")
        ...
        report = {"version": v_id, "parent": self.current, "input_hash": model.source_hash, "output_hash": digest(output), "plan_hash": plan.plan_hash, "engine_version": ENGINE_VERSION, "created_at": datetime.now(timezone.utc).isoformat(), "plan": frozen, "operations": operations, "checks": checks, "environment": env, "font_warnings": source_font_warnings(model, env), "pages_before": before["pages"], "pages_after": after["pages"], "conversion": self.conversion.copy()}
        protections = [{**p, "revision": v_id} for p in frozen["protections"]]
        version = Version(v_id, f"{stem}_{v_id}.docx", output, self.current, plan.plan_hash, encode(report), encode(protections), encode(model.role_overrides))
